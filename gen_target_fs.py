...
def parse_args():
    p = argparse.ArgumentParser("Precompute and cache focal-stack target (amplitude).")
    p.add_argument("--mi_T", type=int, default=96)
    p.add_argument("--z_list_m", type=str, default=None,
                   help="Comma-separated depths with unit given by --z_unit.")
    p.add_argument("--z_unit", type=str, default="m", choices=["m","cm","mm"],
                   help="Unit for z values (default: meters).")
    p.add_argument("--wavelength", type=float, default=5.32e-7, help="meters")
    ...
    p.add_argument("--asm_dy", type=float, default=8e-6, help="meters")
    p.add_argument("--out_dir", type=str, default="outputs_fsopt")
    p.add_argument("--preview_dir", type=str, default=None)   # legacy
    p.add_argument("--data_root", type=str, default=None)     # NEW
...
def main():
    ...
    cache_path = os.path.join(
        args.out_dir, f"target_amp_{args.color}_Tref{args.mi_T}_D{len(z_list)}.pt"
    )
    os.makedirs(args.out_dir, exist_ok=True)
    ...
    # allow None for mipnerf because loader hardcodes root internally
    if data_root is None and args.loader != "mipnerf" and args.scene is None:
        raise SystemExit("Need --data_root (or --preview_dir) for blender data.")
